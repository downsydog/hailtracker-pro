...
import { portalApi, PersonalizedFlyer, FlyerAnalytics } from '@/api/portal'
            ...
            <Card key={flyer.id} className="overflow-hidden">
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <Badge className={FLYER_TYPE_COLORS[flyer.type || 'promo']}>{flyer.type || 'promo'}</Badge>
                </div>
                <p className="text-sm text-muted-foreground mb-4">{flyer.description}</p>
                <Button
                  className="w-full"
                  onClick={() => generateMutation.mutate({ flyer_id: flyer.id })}
