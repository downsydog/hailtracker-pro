...
export function PortalPaymentsPage() {
  ...
  const totalPending = pendingInvoices.reduce((sum, i) => sum + (i.balance_due ?? 0), 0)
              ...
              >
                <div>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm">
                    ...
                    <span>Paid: ${(invoice.amount_paid ?? 0).toLocaleString()}</span>
                  </div>
                </div>
          ...
          {payments.length > 0 ? (
            <div className="divide-y">
              {payments.map((payment) => {
                const Icon = paymentMethodIcons[payment.method ?? 'other'] || DollarSign
