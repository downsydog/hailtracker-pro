...
class SmartDiscoveryService:
    ...
    def discover(
        ...
        # ====================================================================
        if self.here.is_configured():
            log_progress("HERE", "Searching priority categories...")
            try:
                results = self.here.search_priority_categories(lat, lon, radius_meters)
        ...
        # ====================================================================
        if self.google.is_configured():
            log_progress("Google", "Searching priority keywords...")
            try:
                results = self.google.search_priority_categories(
                    lat, lon, radius_meters,
                    enrich_contacts=enrich_google_contacts,
                    enrich_limit=google_enrich_limit
                )
        ...
        # ====================================================================
        if self.tomtom.is_configured():
            log_progress("TomTom", "Searching priority terms...")
            try:
                results = self.tomtom.search_priority_categories(lat, lon, radius_meters)
